...
//class Solution {
//private:	
//	unordered_multimap<string, int> dist_dict_;
//	queue<int> q_;
//	vector<int> dist_;
...
//		{
//			for (int i = 0; i < wordList[widx].length(); i++)
//			{
//				key = wordList[widx];
//				key[i] = '?';
//				dist_dict_.insert(make_pair(key, widx));
//			}
//		}
//	}
//
//	int DealDist1Word(const string &beginWord, const string &endWord, const vector<string>& wordList, int curLen) 
//	{	
//		string key;		
...
//			key = beginWord;
//			key[i] = '?';
//			auto it = dist_dict_.equal_range(key);
//			auto it1 = it.first;
//			while (it1 != it.second)
//			{					
//				if (wordList[(*it1).second] == endWord)				
//					return curLen + 1;				
//				else
//				{
//					if (dist_[(*it1).second] != 0)
//					{
//						it1++;
//						continue;
//					}
//					dist_[(*it1).second] = curLen + 1;
//					q_.push((*it1).second);
//				}				
//				it1++;
//			}
//			dist_dict_.erase(it.first, it.second);
//		}
//		return 0;
//	}
//	
...
//			return 1;
//		
//		dist_ = vector<int>(wordList.size());
//		visited_ = vector<bool>(wordList.size());
//		buildDictionary(wordList);
//		int idx, len;
//
//		//finish the first round by hand.
//		len = DealDist1Word(beginWord, endWord, wordList, 1);
//		if(len != 0)
...
//			q_.pop();
//
//			if (visited_[idx] == false)
//				visited_[idx] = true;
//			else
//				continue;
//			
//			len = DealDist1Word(wordList[idx], endWord, wordList, dist_[idx]);
//			if (len != 0)
//				return len;
//		}
//		return 0; //fail case
...
#pragma region brilliant_bidirectional_BFS_on_leetcode
//class Solution {
//public:
//	int BFSSearch(string beginWord, string endWord, unordered_map<string, pair<int, int>>& wordMap)
//	{
//		queue<string> visitqueue;
//		queue<string> endqueue;
//		visitqueue.push(beginWord);
//		endqueue.push(endWord);
//		while ((!visitqueue.empty()) && (!endqueue.empty()))
//		{
//			auto s = visitqueue.front();
//			int distance = wordMap[s].first;
//			visitqueue.pop();
...
//					{
//						candidate[i] = j;
//						if (wordMap.count(candidate) && !wordMap[candidate].first)
//						{
//							if (wordMap[candidate].second)
//							{
//								return distance + wordMap[candidate].second;
//							}
//							visitqueue.push(candidate);
//							wordMap[candidate].first = distance + 1;
//						}
//					}
//				}
//			}
//			s = endqueue.front();
//			distance = wordMap[s].second;
//			endqueue.pop();
//			for (int i = 0; i<s.size(); i++)
//			{
//				string candidate = s;
...
//					{
//						candidate[i] = j;
//						if (wordMap.count(candidate) && !wordMap[candidate].second)
//						{
//							if (wordMap[candidate].first)
//							{
//								return distance + wordMap[candidate].first;
//							}
//							endqueue.push(candidate);
//							wordMap[candidate].second = distance + 1;
//						}
//					}
//				}
//			}
...
//		for (auto & s : wordList)
//		{
//			wordMap[s] = make_pair(0, 0);
//		}
//		if (wordMap.count(endWord) == 0)
//			return 0;
//		wordMap[endWord].second = 1;
//		wordMap[beginWord].first = 1;
//		return BFSSearch(beginWord, endWord, wordMap);
//	}
//};
#pragma endregion

#pragma region rewrite_bidirection_answer
class Solution {
public:
	int ladderLength(string beginWord, string endWord, vector<string>& wordList)
	{
		...
		unordered_map<string, pair<int, int>> dist_table;
		for (auto s : wordList)
			dist_table[s] = make_pair(0, 0);

		if (dist_table.count(endWord) == 0)
			return 0;

		dist_table[beginWord].first = 1;
		dist_table[endWord].second = 1;

		queue<string> fq; //forward queue 
		queue<string> bq; //backward queue
		...
		string key, mkey;
		int distance;
		while (fq.empty() == false && bq.empty() == false)
		{
			key = fq.front();
			fq.pop();
			distance = dist_table[key].first;
			for(int i=0 ; i< key.length(); i++)
			{
				...
				for (char c = 'a'; c <= 'z'; c++)
				{
					...
					{
						mkey[i] = c;
						if (dist_table.count(mkey) != 0 && dist_table[mkey].first == 0)
						{
							if (dist_table[mkey].second != 0)
								return distance + dist_table[mkey].second;
							else
							{
								dist_table[mkey].first = distance + 1;
								fq.push(mkey);
							}								
						}
					}
			...
			key = bq.front();
			bq.pop();
			distance = dist_table[key].second;
			for (int i = 0; i< key.length(); i++)
			{
				mkey = key;
				for (char c = 'a'; c <= 'z'; c++)
				{
					...
					{
						mkey[i] = c;
						if (dist_table.count(mkey) != 0 && dist_table[mkey].second == 0)
						{
							if (dist_table[mkey].first != 0)
								return distance + dist_table[mkey].first;
							else
							{
								dist_table[mkey].second = distance + 1;
								bq.push(mkey);
							}
						}
					}
...
void q0127_tester()
{
	...
	//cout << s.ladderLength(begin, end, wordlist) << endl; //output : 1

	begin = "hit";
	end = "cog";
	wordlist = {"hot", "dot", "dog", "lot", "log", "cog"};
	//cout << s.ladderLength(begin, end, wordlist) << endl; //output : 5

	wordlist = { "hot", "dot", "dog", "lot", "log"}; 
	//cout << s.ladderLength(begin, end, wordlist) << endl; //output : 0
	...
	cout << s.ladderLength(begin, end, wordlist) << endl; //output : 11
}